...
export function windowScroll(obj, behavior = "smooth"){
 let objData = { ...obj,behavior };
 if(!obj){
   objData = { top : 0,behavior };
 }
 window.scrollTo(objData);
}
...
export function getAge(dateString) {
 let today = new Date();
 let dt;
 if(dateString instanceof Object){
   dt = dateString;
 }else{
   let parseDt = dateString.split('-'); 
   if(isNaN(Date.parse(dateString))){
     dt = `${parseDt[2]}-${parseDt[1]}-${parseDt[0]}`;
   }else{
     dt = dateString;
   }
 }
